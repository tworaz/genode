...
template <typename T>
inline T read_prop_native_value(const fdt32_t* data)
{
	T ret = 0;
	for (size_t i = 0; i < sizeof(T) / 4; ++i) {
		fdt32_t w = fdt32_to_cpu(data[i]);
		ret |= w << 32 * (sizeof(T) / 4 - i);
	}
	return ret;
}
                                      ...
                                      Device_list &out_list)
{
	...
	if ((address_cells * sizeof(uint32_t)) != sizeof(addr_t) ||
	    (size_cells * sizeof(uint32_t)) != sizeof(size_t)) {
	...
	fdt_for_each_subnode(current_node, dtb, ROOT_NODE) {
		...
		addr_t base = read_prop_native_value<addr_t>(reg_data);
		size_t size = read_prop_native_value<size_t>(reg_data + address_cells);
