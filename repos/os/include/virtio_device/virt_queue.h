...
class Virtio::Queue
{
	...
	protected:
		...
		{
			auto const used_idx = (_used->idx % _queue_size);
			auto const avail_idx = (_avail->idx % _queue_size);
	...
	public:
		...
		bool has_used_buffers() const { return _last_used_idx != (_used->idx % _queue_size); }

		void ack_all_transfers() { _last_used_idx = _used->idx % _queue_size; }
		...
		void read_data(FN const &fn)
		{
			...
			Genode::uint16_t const idx = _last_used_idx;
			...
			if (fn(header, data, data_size)) {
				_last_used_idx = (_last_used_idx + 1) % _queue_size;
		...
		void print(Genode::Output& output) const
		{
			...
			Genode::print(output, _avail->idx % _queue_size);
			Genode::print(output, ", used idx = ");
			Genode::print(output, _used->idx % _queue_size);
		...
		: _queue_size(queue_size),
		  ...
		  _ram_ds(ram, rm, _ds_size(queue_size, buffer_size))
		{
			_init_tables();
