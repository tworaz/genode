...
/**
 * This class implements VirtIO network device as defined in section 5.1 of
 * "Virtual I/O Device (VirtIO) Version 1.0" specification. This document can be
 * found at: http://docs.oasis-open.org/virtio/virtio/v1.0/virtio-v1.0.html.
 */
class Virtio_nic::Device
{
	public:

		struct Delegate : Genode::Interface
		{
			virtual bool receive_data(char const *data, size_t size) = 0;
	...
	private:
		...
		/**
		 * Each VirtIO queue contains fixed number of buffers. The optimal size of the buffer
		 ...
		 * available ring and used ring. The default VirtIO queue param values visible below have
		 * been selected to make Ram_dataspace used by both TX and RX VirtIO queues consumealmost
		 * precisely 32Kb of RAM.
		 */
		...
		Genode::Signal_handler<Virtio_nic::Device> _irq_handler;
		bool                                       _link_up = false;
		...
		Virtio::Device_client _init_device()
		{
			...
			_bus.with_upgrade([&] () {
				device_cap = _bus.first_device(Virtio::Device_type::NIC);
			});

			if (!device_cap.valid()) {
				warning("No VirtIO NIC device found!");
		...
		static Nic::Mac_address _read_mac_address(Virtio::Device_client &device)
		{
			Nic::Mac_address mac = {};
		...
		void _handle_irq()
		{
			...
			if (reasons & IRQ_USED_RING_UPDATE) {
				...
				while (_rx_vq.has_used_buffers()) {
					auto rcv = [&] (Virtio_net_header const &,
					                char              const *data,
					                size_t                   size) {
						return _delegate.receive_data(data, size); };
					_rx_vq.read_data(rcv);
				}
			}
	...
	public:
		...
		const Nic::Mac_address &mac_address() const { return _hw_features.mac; }
		...
		bool send_data(char const *data, size_t size)
		{
			...
			if (!_tx_vq.write_data(hdr, data, size, false)) {
				warning("Failed to push packet into tx VirtIO queue!");
				return false;
			}
			_device.notify_buffers_available(TX_VQ);

			/* In many cases the response is available immediately. */
			if (_tx_vq.has_used_buffers())
				_tx_vq.ack_all_transfers();
		...
		Device(Genode::Env                  &env,
		       Virtio::Connection           &bus,
		       Delegate                     &delegate,
		       Genode::Xml_node       const &xml)
		try : _verbose(xml.attribute_value("verbose", false)),
		      _bus(bus),
		      _delegate(delegate),
		...
		~Device() { _device.set_status(Virtio::Device::Status::RESET); }
				      ...
				      private Device::Delegate
{
	private:
		...
		bool _send()
		{
			...
			if (!_device.send_data(data, packet.size()))
				return false;
		...
		bool receive_data(const char *data, size_t size) override
		{
	...
	public:
		...
		bool link_state() override { return _device.link_up(); }
		...
		Session_component *_create_session(const char *args) override
		{
			...
			try {
				return new (md_alloc()) Session_component(
					_env, _bus, *md_alloc(), rom.xml(), tx_buf_size, rx_buf_size);
...
struct Virtio_nic::Main
{
	Genode::Env            &env;
	Genode::Heap            heap { env.ram(), env.rm() };
	Virtio_nic::Root        root { env, heap };

	Main(Env &env)
	try : env(env)
	{
		log("--- VirtIO NIC driver started ---");
...
{
	static Virtio_nic::Main main(env);
}
